...
var storage = multer.diskStorage({
  destination: function (req, file, callback) {
    var dir = "./controllers/TestCase Controllers/Python/app";
...
//for first scenario
const uploadR = async (req, res, next) => {
  ...
  upload(req, res, function (err) {
    ...
    function runTestCase(index) {
      ...
      const dockerExec = spawn('docker', ['exec', 'r-rcomp-1', 'Rscript', `${req.files[0].originalname}`]);
...
const getOutputR = async (req, res, next) => {
...
module.exports = { uploadR , getOutputR};
