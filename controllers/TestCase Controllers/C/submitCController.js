  ...
  upload(req, res, function (err) {
    ...
    function runTestCase(index) {
      ...
      dockerExec.on("close", (code) => {
        ...
        const cleanedActualOutputLines = actualOutputLines.filter(
          (line) => line.trim() !== ""
        );
        ...
        if (passed) {
          for (let i = 0; i < expectedOutputLines.length; i++) {
            if (
              expectedOutputLines[i].trim() !==
              cleanedActualOutputLines[i].trim()
            ) {
              passed = false;
              break;
            }
  ...
  upload(req, res, function (err) {
    ...
    function runTestCase(index) {
      ...
      dockerExec.stdout.on("data", (data) => {
        ...
        if (
          OutputArray[index].length > 0 &&
          OutputArray[index][OutputArray[index].length - 1] === ""
        ) {
